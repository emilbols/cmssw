...
void DeepFlavourTagInfoProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (std::size_t jet_n = 0; jet_n <  jets->size(); jet_n++) {
    ...
    for (const auto & sv : svs_sorted) {
      if(flip_){
	//	edm::LogWarning ("DR") << Geom::deltaR(sv.position() - pv.position(), flip_ ? -jet_dir : jet_dir);
	//edm::LogWarning ("Angle") << btagbtvdeep::vertexDdotP(sv,pv);
      }
      // std::cout << btagbtvdeep::vertexDdotP(sv,pv) << std::endl;
      //if(flip_ && ((btagbtvdeep::vertexD3d(sv,pv).value()/btagbtvdeep::vertexD3d(sv,pv).error()) > 20.0)){continue;}
  ...
  for (unsigned int i = 0; i <  jet.numberOfDaughters(); i++){
    ...
    if (cand->charge() != 0) {
      ...
      bool keep = true;
      if(flip_ && (trackinfo.getTrackSip3dSig() > 10.0)){keep=false;}
      if(keep){
